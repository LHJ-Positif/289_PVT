...
def main():
    ...
    train_file = os.path.join(conf.input['feature_path'],
                              '289_h5py_{}/train_dataset.h5'.format(conf.training_param['visual_encoder_type']))
    scaler_path = os.path.join(conf.input['feature_path'],
                              '289_h5py_{}/feature_scaler.h5'.format(conf.training_param['visual_encoder_type']))
    test_file = os.path.join(conf.input['feature_path'],
                              '289_h5py_{}/test_dataset.h5'.format(conf.training_param['visual_encoder_type']))
    ...
    # Log which visual encoder type is being used
    print(f"Using visual encoder type: {conf.training_param['visual_encoder_type']}")
...
if __name__ == "__main__":
    ...
    if conf.training_param['wandb_ok']:
        wandb.init(project="SELD_Segmentation", 
                   name="0722",
                #    resume="must",
                #    id = "dwkcw6jr"
                #    config=conf,
